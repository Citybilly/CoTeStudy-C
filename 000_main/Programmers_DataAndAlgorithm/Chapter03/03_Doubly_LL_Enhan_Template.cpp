...
public:
    ...
    public:
        ...
        iterator(Node<T>* p) : ptr(p) {} // 생성자 오버로드
    ...
    : count(0)
{
    ...
    trailer = new Node<T> {T(), NULL, NULL}; // Null 을 가리키는 두개의 노드를 생성
    header->next = trailer;
...
void DoublyLinkedList::insert(const iterator& pos, const T& val)
{
    Node<T>* p = pos.ptr;
    Node<T>* new_node = new Node {val, p->prev, p}; // {값, Prev, Next}
...
{
    Node<T>* curr = header->next; // 헤더 다음 노드를 가리킴
